...
private:
	...
	sensor* getSensonrIndex(unsigned sensor_code) {
		for (sensor*& sensor : list_sens) {						// в цикле по списку датчиков
	...
	int getSubsystemIndex(unsigned subsystem_code) {
		int i = 0;
		for (subsystem*& subsystem : list_subsys) {				// в цикле по списку подсистем
			if (subsystem->code == subsystem_code) return i;	// возвращаем не указатель, а его номер в списке
			i++;
		}
		return -1;
	}
...
public:
	...
	static unsigned inputCode(char code_length) {
		...
		sprintf_s(err_message, "Вводимое значение должно быть %d-значным целым числом", code_length); // сохраняет последовательнось символов и значений
		if (res.length() != code_length || res[0] == '0') throw err_message;// если длина шифра не равна нужной, сразу выдаём ошибку
		unsigned long tmp;
		try {
	...
	void printSubsysNoSens() {
		...
		for (subsystem*& subsys : list_subsys) {				// проходим по списку подсистем
			if (!getLink(idx)) {									// если не нашли индексную запись(связку) для подсистемы
	...
	void printSensors() {
		if (list_sens.begin() != list_sens.end()) {			// если список датчиков не пуст
			cout << "Датчики:" << endl;
			cout << left << setw(20) << "Шифр" << setw(40) << "Название" << endl;
			for (sensor*& sensor_ptr : list_sens) {				// в цикле по списку
